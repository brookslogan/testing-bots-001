...
# rely on GitHub's scrubbing. Use --fail-with-body to actually get a sensible
# exit status on errors, and --location to follow redirects + get a sensible
# output status on redirects, including when default max-redirs (50) is hit.
curl --fail-with-body --location --request POST "$SECRET_STALE_QUESTION_NOTIFY_WEBHOOK"
